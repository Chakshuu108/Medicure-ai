...
def _ensure_reply(text: str, user_message: str) -> str:
    ...
    if cleaned:
        return cleaned
    if _emergency_reply(user_message):
        return _emergency_reply(user_message)
    if _simple_reply(user_message):
        return _simple_reply(user_message)
    msg = user_message.lower()
    if any(w in msg for w in ["book", "slot", "appointment"]):
        return (
            "I was not able to book that slot automatically. "
            "Please say \"book slot 1\" or \"book the first slot\" — "
            "or paste the Slot ID directly from the list above."
        )
    return (
...
CRITICAL RULE: For ANY question about disease, condition, medicines, prescriptions, health status, symptoms, or anything personal about the patient — you MUST call get_patient_context and get_prescriptions tools FIRST before replying. Never answer from memory alone.

Examples that require tool calls first:
- "what disease do I have" -> call get_patient_context, then answer
- "what medicines am I on" -> call get_prescriptions, then answer
- "what did the doctor prescribe" -> call get_prescriptions, then answer
- "tell me about my condition" -> call get_patient_context, then answer

Be empathetic and precise. For symptoms, assess urgency and mention if they should seek immediate care.
Always remind patients this is informational, not a diagnosis.""")
    messages = [system] + state["messages"] + [HumanMessage(content=state["user_message"])]
